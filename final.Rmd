...
sim_sir <- function(g,beta,mu,seeds){
  ...
  while(sum(state==1)>0){
    ...
    if(sum(state ==0) >0 & sum(state == 1) > 0) {
    contacts <- adjacent_vertices(g, infected) %>% 
      unlist() %>% 
      names() %>% 
      sub(patter = "\\.", replacement =  "%") %>% 
      data.frame() %>% 
      separate(".", c("vi", "vj"), sep = "%") %>% 
      mutate(vi_id = lapply(vi, function(x){id <- which(V(g)$name == x)}) %>% unlist(),
             vj_id = lapply(vj, function(x){id <- which(V(g)$name == x)}) %>% unlist(),
             weight = E(g)[vi_id %->% vj_id]$weight) %>% 
      filter(vj_id %in% susceptible) %>% 
