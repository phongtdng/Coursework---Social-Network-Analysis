...
With the results of part b) train a model that predicts that time to infection of a node using their

Note, this is the code from b. But the realization_at and realization_bt are later transformed so running here again
...
#Below treshold
table_d_bt <- degree(g) %>% enframe(name = "inf",value="degree") %>%
  merge(realization_bt) 
table_d_bt %>% 
  ggplot(aes(x=t,y=degree)) + geom_point() + scale_y_log10()
...
#Below treshold
table_c_bt <- closeness(g) %>% enframe(name = "inf",value="closeness") %>%
  merge(realization_bt) 
table_c_bt %>% 
  ggplot(aes(x=t,y=closeness)) + geom_point() + scale_y_log10()
```
...
#Below treshold
table_b_bt <- betweenness(g) %>% enframe(name = "inf",value="betweenness") %>%
  merge(realization_bt) 
table_b_bt %>% 
  ggplot(aes(x=t,y=betweenness)) + geom_point() + scale_y_log10()
```
...
#Below treshold
table_pr_bt <- page_rank(g)$vector %>% enframe(name = "inf",value="page_rank") %>%
  merge(realization_bt) 
table_pr_bt %>% 
  ggplot(aes(x=t,y=page_rank)) + geom_point() + scale_y_log10()
```
...
#Above treshold
table <- merge(table_b_at, table_c_at)
table <- merge(table, table_d_at)
table <- merge(table, table_pr_at)
cor(table$t,table[,-c(1,2)])

#Below treshold

table_bt <- merge(table_b_at, table_c_at)
table_bt <- merge(table_bt, table_d_at)
table_bt <- merge(table_bt, table_pr_at)
cor(table$t,table[,-c(1,2)])

```

Use that model to select the seed nodes as those with the smallest time to infection in part c).

-   Choose a β above β~c~. Using centrality, communities or any other metric, find a better set of 1% of seeds in the network so we get more infected people than the random case. Measure the difference of your choice with the random case as:
...
#As degree had the strongest correlation with time, degree has been chosen 
seeds_degree <- degree(g) %>% enframe(name = "inf",value="degree") %>%
  slice_max(degree,n=round(vcount(g)*0.01))

seeds_degree$inf <- as.numeric(seeds_degree$inf)

realization_targeted_degree <- sim_sir(g,beta = 0.3,mu=0.1,seeds_degree$inf) %>%
  group_by(t) %>% summarize(ninf=n())

ggplot() + geom_line(data=realization_random,aes(x=t,y=ninf,col="Random"))+
  geom_line(data=realization_targeted_degree,aes(x=t,y=ninf,col="Targeted (degree)"))
...
# immunization by closeness
set.seed(4785)
vaccinated_degree<- degree(g) %>% enframe(name = "inf",value="degree") %>%
  slice_max(degree,n = round(vcount(g)*0.05))

gp_degree<- delete_vertices(g,vaccinated_degree$inf)
seeds <- sample(1:vcount(gp_degree),vcount(gp_degree)*0.01)

vaccination_targeted_degree <- sim_sir(gp_degree,beta = beta_at,mu=0.1, seeds) %>% 
  group_by(t) %>%
  summarize(ninf=n())

ggplot() + 
  geom_line(data=vaccination_random,aes(x=t,y=ninf,col="Vacc. Random"))+
  geom_line(data=vaccination_targeted_degree,aes(x=t,y=ninf,col="Vacc. Targeted Degree"))

vaccination_random$ninf %>% sum()
vaccination_targeted_degree$ninf %>%  sum()

max(vaccination_random$ninf)
max(vaccination_targeted_degree$ninf)
