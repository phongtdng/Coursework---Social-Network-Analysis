...
data <- read_csv("migration_data.csv") %>% 
  ...
  mutate(migration_2000 = as.numeric(migration_2000)) %>% 
...
realization_at <- sim_sir(g,beta = 0.05,mu=0.1,seeds) 
realization_at
```
...
vaccination_random <- sim_sir(gp,beta = beta_at,mu=0.1,seeds) %>%
  group_by(t) %>% summarize(ninf=n())
```
...
vaccinated_closeness <- closeness(g) %>% enframe(name = "inf",value="closeness") %>%
  slice_max(closeness,n=vcount(g)*0.05)
gp_closeness <- delete_vertices(g,vaccinated_closeness)
seeds <- sample(1:vcount(gp_closeness),vcount(gp_closeness)*0.01)
vaccination_targeted_closeness <- sim_sir(gp_closeness,beta = beta_at,mu=0.1,which(V(gp_closeness)$name == vaccinated_closeness$inf)) %>%
  group_by(t) %>% summarize(ninf=n())

# immunization by betweenness
vaccinated_between <- betweenness(g) %>% enframe(name = "inf",value="betweenness") %>%
  slice_max(closeness,n=vcount(g)*0.05)
gp_between <- delete_vertices(g,vaccinated_between)
vaccination_targeted_between <- sim_sir(gp_between,beta = beta_at,mu=0.1,which(V(gp_between)$name == vaccinated_between$inf)) %>%
  group_by(t) %>% summarize(ninf=n())

# immunization by page rank
vaccinated_pagerank <- page_rank(g)$vector %>% enframe(name = "inf",value="page_rank") %>%
  slice_max(closeness,n=vcount(g)*0.05)
gp_pagerank <- delete_vertices(g,vaccinated_pagerank)
vaccination_targeted_pagerank <- sim_sir(gp_pagerank,beta = beta_at,mu=0.1,which(V(gp_pagerank)$name == vaccinated_pagerank$inf)) %>%
  group_by(t) %>% summarize(ninf=n())
```
