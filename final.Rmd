...
author: "Phong Duong, Nienke Visscher, Frederick Peña Sims"
date: "2024-05-18"
...
The network that is used for this report is made from a bilateral network of global migration database for the period 1960 - 2000. Although the data does not have the highest freshness, in that it is not the most up-to-date, it is one of the first comprehensive database of global bilateral migration for the last half of the 20th century.

In this report, we will focus on the migration during the year 2000. We will create a simulation of information propagation to see how information would spread from countries to countries based on the amount of immigrants between them. We will then perform various seeding strategies to see which metric will accelerate the information spreading the most. We will also simulate various "vaccination" strategies in which we will select a group of countries and eliminate their ability to spread information in order to study the most effective group against information dissemination. The report will be concluded with a machine learning model in which different metrics are used to create a predictive model for the time it takes to spread information to countries.
...
We also visualized the network to have a brief ideas of how it looks like. We can see that it is quite a dense network with few nodes and a lot of edges. This will undoubtedly make the spreading of information (or infection) very rapid.
...
From the visualization, we can see that the information spread quickly at the beginning and peaked at around 43 new infected node around time *t* = 2. Since a large portion of the network has already been infected at this point, the amount of recovered country should have also gone up. The spreading starts to subside and around time *t* = 10 perhaps due to the fect that there are less susceptible nodes and that the majority of the nodes have already been spread the information to.
...
## 4 Vaccination Simulation

### 4.1 Vaccinating 5% randomly

d)  Choose those 5% randomly in the network. Simulate the SIR model above β~c~ using 1% of the remaining nodes as seeds. Choose those seeds randomly.

In order to create the effect of a reliable vaccine stopping the infection and transmission of information, we delete 5% of the nodes in the network randomly. As such, they will lose their function as neighbors, and therefore possible recipients of the information reducing the overall connectedness of the network. To run the simulation, we use the epidemic threshold β above β~c~ , which resulted in a higher infrection rate as outlined earlier.
...
distinct(vaccination_random, inf)
```

Running the model, we see that the resulting table consists of 215 rows. Since 5% of all 226 nodes were vaccinated (deleted), this means that at some time *t* all remaining nodes (countries) have been infected.

```{r}
vaccination_random <- vaccination_random |> 
  group_by(t) |> summarize(ninf=n())
ggplot() + 
  geom_line(data=vaccination_random,aes(x=t,y=ninf,col="Vacc. Random"))
```

As the visualization shows, information spread as quick as the same simulation in chapter `Above Treshold` peaking even at 51 infections after *t* = 2. Also mirroring the results of the aforementioned chapter, spreading slowed down due to the increase in recovered and subsequent decrease in susceptible nodes. The results are as expected insofar as spreading will occur via non-deleted nodes since direct migration between is highly likely for any combination of remaining nodes.

### 4.2 Vaccination according to their centrality

e)  Choose those 5% according to their centrality. Simulate the SIR model above using 1% of the remaining nodes as seeds. Choose those seeds randomly.

Following, we will perform targeted vaccination, where seeds who are subsequently deleted to simulate their vaccination are sampled according to the centrality measures closeness, betweeness, and page rank. The top 5% of nodes according to the respective centrality measure are then sampled and deleted.
...
```{r}
vaccination_targeted_closeness <- vaccination_targeted_closeness %>% 
  group_by(t) %>%
  summarize(ninf=n())
vaccination_targeted_between <- vaccination_targeted_between %>%
  group_by(t) %>% summarize(ninf=n())
vaccination_targeted_pagerank <- vaccination_targeted_pagerank %>%
  group_by(t) %>% summarize(ninf=n())
...
Visualizing the trend of the spread, we see that vaccinating (deleting) nodes from the network according to their `closeness` results in even more adverse time to infection peak reaching it at *t* = 1. On the other hand, vaccinating nodes according to `page rank` sees not only a postponement in reaching the infection peak but also reduces the number at the peak of infection to 48. This may be attributable due to vaccinating the most central nodes according to their neighbors' importance in the network. As such, the transmission of information may be slightly delayed due to vaccinated nodes initially connecting lesser important nodes with more important nodes. On the other hand, closeness as a measure may undervalue the importance of other nodes in such a highly connected network. Lastly, distributing vaccinations according to `betweeness` almost perfectly mirrors the spread trend if nodes are vaccinated at `random`.
...
No matter the centrality used to determine vaccinated nodes, the information spread to all 215 remaining nodes. As such, the results mirror earlier results where infection was realised by value of centrality and the information also spread to all nodes in the network.

-   The difference in the time of the peak of infection (when most infections happen).

```{r}
rbind(
  slice_max(vaccination_random, ninf),
  slice_max(vaccination_targeted_between, ninf),
  slice_max(vaccination_targeted_closeness, ninf),
  slice_max(vaccination_targeted_pagerank, ninf)
) %>% cbind(target_by = c("random", "between", "closeness", "page rank"))
```

Turning to the time it took to hit the peak in number of infected nodes, we reiterate our findings due to the visual cues in the last graph. While vaccinating nodes according to `closeness` shortens the time until the largest number of nodes are infected at time *t* and increases the absolute number to 54, `random` vaccination and vaccinating according to `betweeness` mirror each others peak (51) and time to reach said peak (*t* = 2). Using `page rank` as the basis to vaccinate nodes extends the time to hit the infection peak to *t* = 3 and reduces the absolute number at the peak of infection to 48.

## 5 Comparing the Role of Centrality in Infection and Vaccination

Generally, and according to intuition, the effect of the centrality on the spreading is inverted when comparing infection and centrality. Whereas infecting the network based on `page rank` leads to faster spread, vaccinating nodes based on the same centrality measure slows down the speed of contamination comparatively. This, of course, closely follows intuition where infection and vaccination exert contrary effects and subsequently sampling the same nodes for these inverted purposes generates these inverted outcomes. It is interesting, however, how much greater the spread of the absolute number of infected nodes at the peak infection point *t* is for infecting nodes randomly and based on centrality measures (43-58) versus performing the same technique to identify even more nodes selected to be vaccinated (48-53). As such, one conclusion may be that spreading information in a network may be increased more easily with measures of centrality with fewer initially infected nodes than stopping (or slowing down) the spread of information even if using measures of centrality and more initially vaccinated nodes.
...
### 6.2 Applying seeds with lowest time to infection
...
The graphs show a highly similar course of infections. However, the targeted simulation shows a higher peak of infections earlier in the simulation compared to the random simulation. Also, the targeted simulation has higher maximum number of infections (difference of 7 infections) at a one time point then the random simulation. However, in both simulation the entire network ends up infected.

### 6.3 Vaccination according to time to infection
