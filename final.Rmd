...
In order to create the effect of a reliable vaccine stopping the infection and transmission of information, we delete 5% of the nodes in the network randomly. As such, they loose their function as neighbors, and therefore possible recipients of the information reducing the overall connectedness of the network. To run the simulation, we use the epidemic threshold β aboveβ~c~ , which resulted in a higher infrection rate as outlined earlier.
...
Following, we perform targeted vaccination, where seeds who are subsequently deleted to simulate their vaccination are sampled according to the centrality measures closeness, betweeness, and page rank. The top 5% of nodes according to the respective centrality measure are then sampled and deleted.
...
df <- merge(df, table_d_at)
df <- merge(df, table_pr_at)
cor(df$t,df[,-c(1,2)])
```
